...
- `scanWindowSize` option on `StreamClassifierOptions` — scans trailing window (default 1024) per `feed()` call, keeping total work O(n) instead of O(n²).

**Adapter timeouts**
- All built-in adapters (`createOpenAIAdapter`, `createOllamaAdapter`, `createAnthropicAdapter`, `createOpenAICompatibleAdapter`) now abort requests after 30s by default. Override with `timeoutMs`.

**Named adapter state**
- `id` option on `AsyncClassifierOptions` — explicit string key for cache and circuit-breaker sharing.

**Robust JSON extraction**
- `parseClassifierJson()` now uses a brace-balancing parser instead of a greedy regex.

**True LRU eviction**
- `LRUCache.get()` updates access order so eviction is genuinely least-recently-used.

### Changed

- Removed re-exports from `classify.ts` and `rules.ts` to eliminate circular dependencies and improve tree-shaking. `index.ts` continues to export all public symbols.

### Fixed

- O(n²) stream classifier performance — `feed()` no longer re-classifies the full accumulated buffer on every chunk.
- Missing timeouts on adapter `fetch()` calls that could stall indefinitely.
- Greedy regex in `parseClassifierJson()` could match across multiple JSON objects.
...
- `classify()` and `classifyWithAdapter()` now detect _all_ rule matches in a single pass, not just the first.
